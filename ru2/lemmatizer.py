...
from spacy.symbols import ADJ, DET, NOUN, NUM, PRON, PROPN, PUNCT, VERB, POS
from spacy.lemmatizer import Lemmatizer
from spacy.compat import unicode_

import os
DEBUG = os.environ.get("DEBUG_LEMMATIZER", False)

from spacy import symbols as symb

orth = {
    getattr(symb, name): name
    for name in dir(symb)
    if type(getattr(symb, name)) == type(0)
}
...
class RussianLemmatizer(Lemmatizer):
    ...
    def __call__(self, string, univ_pos, morphology=None):
        if DEBUG:
            print(
                "Set lemma for selected POS tag: {} {} {}".format(
                    string, orth[univ_pos], orthd(morphology)
                )
            )
    ...
    def is_base_form(self, univ_pos, morphology=None):
        if DEBUG:
            print(
                "Method Lemmatizer.is_base_form is not implemented: {} {}".format(
                    univ_pos, morphology
                )
            )
        # TODO
    ...
    def lookup(self, string):
        if DEBUG:
            print("No tagger, so we do Lemmatizer.lookup: {}".format(string))
        analyses = self._morph.parse(string)
        ...
        if len(list(set(analyses_known))) == 1:
            ...
            print(
                "Found {} lookup alternatives for: {}".format(
                    len(list(set(analyses_all))), string
                )
            )
        return string
